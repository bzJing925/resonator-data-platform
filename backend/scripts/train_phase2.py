...
def train_epoch(dataloader, recon, sampler, criterion, optimizer, device, target_k):
    ...
    for batch in dataloader:
        ...
        batch_size, n_points = target_z.shape
        fs_true = cond[:, 0]
        fp_true = cond[:, 1]
        ...
        for b in range(batch_size):
            ...
            idx = torch.where(y_soft[b] > 1.0 / n_points)[0]
            if len(idx) == 0:
                _, topk_idx = torch.topk(p_norm[b], min(k_actual_b, n_points))
                idx = topk_idx
        ...
        sample_mask = torch.zeros(batch_size, target_k, dtype=torch.bool, device=device)
...
def main():
    ...
    criterion = SparseReconLoss(
        lambda_count=0.1, lambda_smooth=0.01, lambda_phys=0.5, lambda_efficiency=0.5
    ).to(device)
    ...
    if end_epoch >= args.epochs or patience_counter >= 15:
        ...
        with open(output_dir / "config.json", "w") as f:
            json.dump(
                {"piezo_thickness": args.piezo_thickness, "target_k": args.target_k, **config},
                f,
                indent=2,
            )
